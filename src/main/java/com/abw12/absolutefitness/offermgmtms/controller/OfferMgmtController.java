...
import com.abw12.absolutefitness.offermgmtms.service.OfferMgmtService;
...
public class OfferMgmtController {
    ...
    @Autowired
    private OfferMgmtService offerMgmtService;
    ...
    logger.info("Inside addOffer Rest API");
     try{
        return new ResponseEntity<>(offerMgmtService.createOffers(request), HttpStatus.OK);
     }catch (Exception e){
    ...
    public ResponseEntity<?> getOfferDetails(@PathVariable String offerId){
        ...
        try{
            return new ResponseEntity<>(offerMgmtService.getOfferDetails(offerId), HttpStatus.OK);
        }catch (Exception e){
